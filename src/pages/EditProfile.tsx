...
const EditProfile = () => {
  ...
  // Helper to check if email is auto-generated from phone login
  const isAutoGeneratedEmail = (email: string | null | undefined): boolean => {
    if (!email) return true;
    return email.endsWith('@phone.grumming.app');
  };
  ...
  const fetchProfile = async () => {
    ...
    if (error) {
      ...
      setPhone(data.phone || '');
      // Only show real emails, not auto-generated phone-based ones
      const profileEmail = data.email || user.email || '';
      setEmail(isAutoGeneratedEmail(profileEmail) ? '' : profileEmail);
      setAvatarUrl(data.avatar_url);
      setEmailVerified((data as Profile).email_verified || false);
    } else {
      const userEmail = user.email || '';
      setEmail(isAutoGeneratedEmail(userEmail) ? '' : userEmail);
    }
    setIsLoading(false);
  };
